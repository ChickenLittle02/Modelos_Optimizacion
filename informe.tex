...
\subsection{Análisis visual de la superficie y curvas de nivel}
A partir del análisis previo, se observa que la región definida por $x \in [-5,5]$ y $y \in [-5,5]$ engloba las zonas críticas más relevantes de la función, ya que incluye tanto regiones convexas como no convexas. Este intervalo de valores de 
$x$ permite capturar de manera adecuada el comportamiento crítico de la función, incluyendo los puntos donde la Hessiana cambia de signo. En consecuencia, dicho dominio resulta representativo para estudiar tanto la existencia de mínimos locales como el desempeño esperado de los métodos de optimización, razón por la cual se decidió analizarlo con mayor profundidad.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{superficie.png}
\caption{Superficie tridimensional de $f(x,y)$.}
\label{fig:superficie}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{curvas_nivel.png}
\caption{Curvas de nivel de $f(x,y)$.}
\label{fig:curvas}
\end{figure}

\subsubsection*{Interpretación de la superficie:}
Segun la Figura 1 la superficie tridimensional de la función revela claramente tres regiones con comportamientos geométricos diferenciados:

\begin{itemize}
    \item Para $x > 2$:  
    En esta zona la función es convexa, lo que se manifiesta como una forma de ``tazón'' bien definido. La curvatura es positiva en todas las direcciones y existe un mínimo local estable hacia el cual convergen los métodos de optimización.

    \item Para $0 < x < 2$:  
    En esta región la superficie se curva hacia arriba en la dirección $y$ (debido al término $y^2$), pero hacia abajo en la dirección $x$. Esto indica la coexistencia de curvaturas de signo opuesto, característica típica de un punto de silla. Este comportamiento coincide con el análisis de la Hessiana, ya que en esta región se cumple $12x^2 - 24x < 0$.

    \item Para $x < 0$:  
    En esta zona la función vuelve a ser convexa. Aunque el mínimo local es menos pronunciado que en la región $x > 2$, la curvatura sigue siendo positiva y el comportamiento es estable.
\end{itemize}

Lo cual confirma que la función no es convexa globalmente, pero sí presenta regiones convexas bien definidas separadas por una zona no convexa.

\subsubsection*{Interpretación de las curvas de nivel:}

Las curvas de nivel proporcionan una visión complementaria del comportamiento de la función, ya que representan los conjuntos de puntos donde $f(x,y)$ toma un valor constante.

\begin{itemize}
    \item Para $x < 0$ y $x > 2$:  
    Las curvas de nivel son aproximadamente elípticas y concéntricas. Esto indica que, al desplazarse desde cualquier punto cercano, el gradiente apunta hacia un punto crítico estable. Geométricamente, este patrón es característico de mínimos locales con curvatura positiva en todas las direcciones.

    \item Para $0 < x < 2$:  
    Las curvas de nivel presentan una forma hiperbólica, típica de regiones tipo silla. En estas zonas no existe una dirección de descenso dominante, lo que genera trayectorias inestables para los algoritmos de optimización.

    \item Además se identifican tres puntos donde las curvas se cierran o presentan un cambio significativo en su geometría, ubicados aproximadamente en $(-0.8,0)$, $(0.65,0)$ y $(2.9,0)$. Estos puntos corresponden a las soluciones del sistema $\nabla f(x,y)=0$. Los puntos $(-0.8,0)$ y $(2.9,0)$ se asocian a mínimos locales, evidenciados por curvas cerradas y concéntricas, mientras que el punto $(0.65,0)$ corresponde a un punto de silla, caracterizado por curvas de nivel de tipo hiperbólico.
...
Los casos con learning rate = 0.001 y 0.005 muestran convergencias extremadamente lentas: 
con iteraciones promedio cercanas o superiores a las 2000 iteraciones. En la mayoría de las regiones,
al utilizar estos learning rates la razón principal de no convergencia es que el algoritmo alcanza
el máximo de 2400 iteraciones sin cumplir los criterios de paro.
...
Al analizar las estadísticas de convergencia para lr = 0.05, el método presenta el comportamiento más estable,
con un número promedio de iteraciones cercano a 1600 y resultados finales consistentes.
Para lr = 0.1, el método muestra una pérdida de estabilidad, con promedios superiores a 2000 iteraciones
y numerosos casos que alcanzan el máximo de iteraciones sin converger.


\subsection*{3. A partir de lr $\geq 0.2$ el método diverge sistemáticamente}
Los datos muestran que el 100\% de los casos con lr = 0.2, 0.3 y 0.5 presentan no convergencia,
mayoritariamente asociada a divergencia por overflow numérico.
Se registraron 2031 ejecuciones no convergentes de un total de 2400 experimentos, equivalente al 84.6\% del total.
Dentro de estas, 900 ejecuciones corresponden exclusivamente a learning rates mayores o iguales a 0.2.
...
\[
f_{\text{final}} \in [-15.2344,\, -1.4457]
\]
...
Con todo lo anterior concluimos que para la ejecucion del algoritmo de gradiente Descendiente
con este problema los datos muestran de manera consistente que:
...
\begin{itemize}
    \item El desempeño más eficiente se obtuvo con \(\alpha_0 = 0.8\) y \(\beta = 0.5\), alcanzando un promedio de aproximadamente 90 iteraciones.
...
De todos los experimentos realizados se decidió escoger y analizar la ejecución
de ambos algoritmos a partir del punto inicial
$x_0 = (1.0227,\,0.3033)$.
Dicho punto fue seleccionado con el objetivo de realizar un análisis exploratorio interesante
pues se encuentra en una región del dominio donde la función presenta alta
curvatura en la variable $x$ y un gradiente de magnitud moderada.

Esta elección permite evaluar de manera clara la sensibilidad de los algoritmos
a la geometría local de la función y a la selección del tamaño de paso.
Asimismo, se consideraron dos valores del learning rate para el método de paso fijo:
$lr = 0.1$ y $lr = 0.3$, con el fin de analizar tanto un escenario moderado como uno
claramente desfavorable para dicho método.


\subsection{Trayectorias sobre curvas de nivel}
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{graficas/trayectorias_lr_0.1.png}
\caption{Trayectorias del GD con paso fijo y del método de Armijo para $lr=0.1$.}
\label{fig:tray_01}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{graficas/trayectorias_lr_0.3.png}
\caption{Trayectorias del GD con paso fijo y del método de Armijo para $lr=0.3$.}
\label{fig:tray_03}
\end{figure}

En las Figuras \ref{fig:tray_01} y \ref{fig:tray_03} se observan las trayectorias
seguidas por ambos métodos sobre las curvas de nivel de la función objetivo.
El comportamiento observado está directamente relacionado con la geometría local
de la función alrededor del punto inicial.

Para $lr=0.1$, el descenso por gradiente con paso fijo presenta una trayectoria
oscilante que no sigue una dirección de descenso consistente.
Esto se debe a que el tamaño de paso constante no se adapta a la curvatura de la
función, provocando que el algoritmo sobrepase repetidamente el valle del mínimo.
En contraste, el método de Armijo ajusta dinámicamente el tamaño de paso en cada
iteración, permitiendo desplazamientos más pequeños en regiones de alta curvatura
y dando lugar a una trayectoria suave y estable hacia una región de mínimo.


Para $lr=0.3$, el método de paso fijo se aleja rápidamente de la región de interés,
ya que el tamaño de paso es excesivo para la magnitud del gradiente en el punto
inicial. Esto provoca actualizaciones demasiado grandes que llevan al algoritmo
fuera del dominio relevante. En contraste, el método de Armijo reduce
automáticamente el tamaño de paso cuando no se satisface la condición de descenso
suficiente, manteniendo una trayectoria controlada y estable hacia el mínimo.


\subsection{Evolución del valor de la función}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{graficas/f_vs_iter_lr_0.1.png}
\caption{Valor de la función objetivo vs iteraciones para $lr=0.1$.}
\label{fig:f_01}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{graficas/f_vs_iter_lr_0.3.png}
\caption{Valor de la función objetivo vs iteraciones para $lr=0.3$.}
\label{fig:f_03}
\end{figure}

La Figura \ref{fig:f_01} muestra que, para $lr=0.1$, el descenso por gradiente con
paso fijo presenta oscilaciones pronunciadas en el valor de la función.
Este comportamiento se debe a que el tamaño de paso constante produce
actualizaciones que alternan entre regiones de mayor y menor valor de la función,
sin garantizar una disminución monótona.

En contraste, el método de Armijo asegura que cada iteración cumpla una condición
de descenso suficiente, lo que se traduce en una disminución estable y progresiva
del valor de la función hasta alcanzar un mínimo local.

Para $lr=0.3$ (Figura \ref{fig:f_03}), el tamaño de paso fijo resulta excesivo para
la región del dominio en la que se encuentra el punto inicial.
Como consecuencia, el algoritmo produce actualizaciones demasiado grandes que
incrementan drásticamente el valor de la función, generando una divergencia
numérica. El método de Armijo evita este comportamiento al reducir el tamaño de
paso cuando el descenso propuesto no es adecuado, manteniendo un comportamiento
estable y decreciente.


\subsection{Norma del gradiente}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{graficas/gradiente_vs_iter_lr_0.1.png}
\caption{Norma del gradiente en escala logarítmica para $lr=0.1$.}
\label{fig:grad_01}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{graficas/gradiente_vs_iter_lr_0.3.png}
\caption{Norma del gradiente en escala logarítmica para $lr=0.3$.}
\label{fig:grad_03}
\end{figure}

En las Figuras \ref{fig:grad_01} y \ref{fig:grad_03} se observa que el método de
Armijo reduce progresivamente la norma del gradiente hasta valores cercanos a
cero, lo que indica convergencia hacia un punto crítico. 
Por el contrario, el descenso por gradiente con paso fijo mantiene normas elevadas
u oscilantes, ya que el tamaño de paso constante no permite una reducción
controlada del gradiente en regiones de alta curvatura.
Para valores grandes del learning rate, como $lr=0.3$, este efecto se amplifica y
la norma del gradiente crece rápidamente, evidenciando la divergencia del método.


\subsection{Limitaciones del descenso por gradiente con paso fijo}
El comportamiento observado del descenso por gradiente con paso fijo no se debe
a errores de implementación, sino a limitaciones inherentes al método cuando se
aplica a funciones no convexas y mal condicionadas.

En particular, la función considerada presenta regiones con curvatura elevada en
la variable $x$ debido al término $x^4$, lo que provoca gradientes de gran magnitud.
Dado que el método utiliza un tamaño de paso constante, no existe un mecanismo que
adapte el desplazamiento a la geometría local de la función, lo que puede conducir
a oscilaciones o divergencia.

Además, el criterio de paro utilizado se basa en el cambio entre iteraciones
consecutivas, lo cual no garantiza una disminución monótona del valor de la función
ni de la norma del gradiente. Este efecto se ve amplificado para valores grandes
del learning rate, como $lr=0.3$, donde el método abandona rápidamente la región
relevante del dominio.

Por estas razones, el descenso por gradiente con paso fijo resulta altamente
sensible a la elección del tamaño de paso y del punto inicial, mientras que el
método de Armijo, al incorporar una búsqueda de línea adaptativa, logra una mayor
estabilidad y robustez en todos los escenarios considerados.
...
En contraste, el método de búsqueda en línea con Armijo logró convergencia en todos los casos donde el gradiente no era numéricamente degenerado. Gracias al ajuste adaptativo del paso, el algoritmo evitó las oscilaciones y la divergencia incluso cuando el valor inicial de $\alpha$ era grande. Los datos muestran que Armijo fue capaz de reducir el paso de manera automática cuando la función no satisfacía la condición de descenso suficiente, garantizando estabilidad global y trayectorias de descenso más controladas.

Asimismo, las conclusiones analíticas derivadas de los datos numéricos se ven plenamente corroboradas por el análisis gráfico.
Las trayectorias sobre las curvas de nivel, la evolución del valor de la función y la norma del gradiente reflejan de manera consistente los fenómenos observados cuantitativamente: oscilaciones y divergencia en el método de paso fijo para tasas grandes, y un descenso monótono, estable y controlado en el método de Armijo.
